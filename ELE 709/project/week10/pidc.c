...
void* controller(void* arg){
    unsigned int k = 0;
    float motor_angle, error, p, i, d, control_signal;
    float prev_error;
    float T = 1.0/Fs;
    
    //assume input is 0 before t = 0 (which makes the system state = 0)
    prev_error = 0;
    i = 0;
    d = 0;
    ...
    for(k = 0; k < samples; k++){
        ...
        error = ref[k] - theta[k];                          //error between output of system and reference input angle
        p = Kp*error;
        i = (Kp/Ti)*T*prev_error + i;         //forward rectangle approximation to area of integral over previous sampling period
        d = (Td/(N*T + Td))*(Kp*N*(error-prev_error) + d);  //backwards Euler method for derivative of error (uses current error and previous error to get slope) for LPF + derivative
        control_signal = p + i + d;                         //control signal to send to motor (how many volts it should receive)
