...
#include "lab4_packet.h"	//defines data packet structure and packet types
...
int sendFile(int sd);   //function declaration for sending file data to a client
    ...
    while(1){       //loops until child exits or server is externally closed
        ...
        switch(fork()){
            case 0:                 //child of parent
                ...
                sendFile(new_sd);   //send message to client
                printf("Closing TCP connection to client\n");
...
int sendFile(int sd){//reads file contents to client
    int n;              //number of bytes that have been read in from socket sd
    struct packet pkt;  //data packet struct
