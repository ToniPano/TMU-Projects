...
int main(int argc, char **argv){
	...
	int this_server_ip, this_server_port;	//ip and port of this peer as the content server
			...
			switch(command){
				...
				case search_pdu:	//'S' = download one specific content
					...
					if(pkt.type == err_pdu){
						...
						if(this_server_ip == *((int *) &(pkt.data[20])) && (short)this_server_port == *((short *) &(pkt.data[24]))){
							printf("This peer (\"%s\") should not try to download \"%s\" from itself\n", peer_name, content_name);
							break;
						}
						...
						if(fd == -1){
							...
							//stay in loop if someone has taken this peer's username already
							while(1){
